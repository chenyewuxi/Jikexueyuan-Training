...
function setSaveLinstener() {
    $("#save").on('click', function(event) {
        ...
        var curtDate = $("#newsaddtime").val();
        var testDate= new Date(Date.parse(curtDate.replace(/-/g,"/")));

        if (isNaN(Date.parse(testDate))){
            alert("建议您使用chrome输入合理的日期格式(YYYY-MM-DD)!");
            return;
        }
        ...
        $.ajax({
            ...
            data: {
                ...
                "news_image": $("#newsimage").val(),
                "news_content": $("#newscontent").val(),
                "news_addtime": testDate.toISOString()
            },
            success: function(data) {
                if (data != "undefined" && data["desc"] == "OK") {
...
function deleteRow(el) {
    var newsid = $(el).parent().siblings('div.news_id').text();
...
function copyFromDetailToForm(el) {
    ...
    $("#hdfnewsid").attr('value', $(parent).siblings('div.news_id').text());
    $("#category").val($(parent).siblings('div.category').text());
    $("#newstitle").attr('value', $(parent).siblings('div.news_title').text());
    $("#newsimage").attr('value', $(parent).siblings('div.news_image').text());
    $("#newscontent").text($(parent).siblings('div.news_content').text());
    $("#newsaddtime").attr('value', $(parent).siblings('div.news_addtime').text().substring(0, 10));
...
function setDetailLinstener() {
    $("#newdetails").on('mouseenter', 'div.row', function(event) {
        // alert("It's me!");
        $(this).addClass('active');
    });

     $("#newdetails").on('mouseleave', 'div.row', function(event) {
        // alert("It's me!");
        $(this).removeClass('active');
    });


    // $("#newdetail").on('mouseover', 'div.row', function(event) {
    //     $(this).addClass('active');
    // });

    // $("#newdetails div.row").hover(function() {
    //     $(this).addClass('active');
    // }, function() {
    //     $(this).removeClass('active');
    // });
